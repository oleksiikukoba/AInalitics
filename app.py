...
CATEGORIES = [
    "Танки", "Артилерія", "Авіація", "Бронетехніка",
    "Дрони", "Піхота і гарячі напрямки",
    "Героїзм та унікальні історії військових, портретні репортажі",
    "Навчання", "Огляди зразків озброєння",
    "Різне"
]
...
@st.cache_data(ttl=86400)  # Кешуємо результат на добу, бо відповідь GPT для того ж тексту буде однакова
def categorize_video_gpt(title, description, categories_list):
    """Категоризує відео за допомогою GPT."""
    if not OPENAI_API_KEY:
        return "Різне"  # Або інша категорія за замовчуванням

    # Обмежуємо довжину опису, щоб економити токени
    description_snippet = description[:1000] if description else ""

    prompt = f"""
    Проаналізуй назву та опис відео з YouTube каналу військової тематики "Армія TV".
    Визнач, до якої з наступних категорій воно найкраще підходить.
    Якщо відео не підходить чітко під жодну з перших 8 категорій, віднеси його до категорії "Різне".
    Поверни ТІЛЬКИ ОДНУ назву категорії зі списку. Не додавай жодних пояснень чи префіксів.
    ...
    try:
        response = openai.chat.completions.create(
            model="gpt-3.5-turbo",  # Або "gpt-4o-mini", якщо доступна і бажаєш кращої якості
            messages=[
                {"role": "system",
                 "content": "Ти помічник, який точно категоризує відео за наданим списком категорій."},
                {"role": "user", "content": prompt}
            ],
            max_tokens=30,  # Достатньо для назви категорії
            temperature=0.1  # Низька температура для більш детермінованих результатів
        )
        ...
        if category in categories_list:
            ...
            # Спроба знайти підрядок, якщо GPT повернув щось зайве
            for cat_option in categories_list:
                if cat_option.lower() in category.lower():
                    return cat_option
            return "Різне"  # Якщо зовсім не те
    except Exception as e:
        st.warning(f"Помилка OpenAI при категоризації відео '{title}': {e}")
        return "Різне"  # У разі помилки API
